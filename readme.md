...
Для этой операции необходимо один раз запустить приложение с флагом `--sync` и дождаться сообщения об успешном запуске сервера. Если операция завершится успешно, в корневой директории создастся файл `sync.json` с данными о синхронизации. При повторном запуске приложения с флагом `--sync` и наличии `sync.json` синхронизация не произведётся и выведется сообщение об ошибке. Это необходимо для предотвращения случайного очищения базы данных, если синхронизация уже производилась. В таком случае всё ещё возможно произвести синхронизацию (например, при повереждении структуры базы данных или обновлении версии приложения), запустив приложение с флагом `--sync --force`, пересоздав базу данных.
